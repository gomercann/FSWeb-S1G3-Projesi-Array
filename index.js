...
function indekstekiCesitiGetir(orijinalTatlar, a) {
  return orijinalTatlar[a];
}
...
function ismeGoreCesitCikar(orijinalTatlar, tat) {
  const dizi = orijinalTatlar;

  dizi.splice(dizi.indexOf(tat), 1);
  return dizi;
}
...
function ismeGoreFiltrele(orijinalTatlar, aramaIfadesi) {
  const icerenTatlar = [];

  for (let i = 0; i < orijinalTatlar.length; i++) {
    if (orijinalTatlar[i].includes(aramaIfadesi)) {
      icerenTatlar.push(orijinalTatlar[i]);
    }
  }
